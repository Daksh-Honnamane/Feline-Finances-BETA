...
def setup_screen():
    name = ""
    types = ["Orange", "Black", "White", "Calico"]
    ...
    while True:
        ...
        draw_text("UP/DOWN select  LEFT/RIGHT change  ENTER confirm", 50, 70)
        ...
        pygame.draw.circle(screen, colors[types[type_index]], (preview_x, preview_y), 40)
        pygame.draw.circle(screen, BLACK, (preview_x - 10, preview_y - 5), 4)
        pygame.draw.circle(screen, BLACK, (preview_x + 10, preview_y - 5), 4)
        if personalities[pers_index] == "Lazy":
            pygame.draw.line(screen, BLACK, (preview_x - 10, preview_y + 10),
                             (preview_x + 10, preview_y + 10), 2)
        else:
            pygame.draw.arc(screen, BLACK,
                            (preview_x - 15, preview_y, 30, 20), 3.14, 0, 2)
...
store_prices = {"food": 5, "toy": 10, "vet": 20}
...
rest_btn = pygame.Rect(270, 500, 100, 40)
clean_btn = pygame.Rect(380, 500, 100, 40)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if in_store:
                ...
                if feed_btn.collidepoint(event.pos) and inventory.get("food", 0) > 0:
                    cat.feed()
                    inventory["food"] -= 1
                ...
                if rest_btn.collidepoint(event.pos):
                    cat.rest()
                if clean_btn.collidepoint(event.pos):
                    cat.clean()
    ...
    right_x = 700
    draw_text(f"Money: ${money}", right_x, 50)
    draw_text(f"Total Spent: ${total_spent}", right_x, 80)
    draw_text(f"Food: {inventory.get('food',0)}", right_x, 110)
    draw_text(f"Toys: {inventory.get('toy',0)}", right_x, 140)
    ...
    draw_button(chore_btn, "Do Chore", check_hover=not in_store and not in_chore)
    draw_button(store_btn, "Store", check_hover=not in_store and not in_chore)
    ...
    if in_chore:
        ...
        if active_minigame == "trash":
            ...
            # Show chore buttons
            draw_text("Do Chore", overlay_x + 20, overlay_y + 12, f=big_font)
    ...
    if in_store:
        ...
        # Title and money
        draw_text("Store", overlay_x + 20, overlay_y + 12, f=big_font)
